...
function callback(error) {
    if (error) {
        logger.encryptLog(error.message || error,3);
        logger.encryptLog('Process exited with error code 1.',3);
        process.exit(1);
    } else {
        logger.encryptLog('Process finished successfully.',3);
        process.exit(0);
    }
}
...
program.command('en')
    ...
    .option('--p.in <file>', 'Set a text file')
    .option('--p.str <file>', 'Set text')
