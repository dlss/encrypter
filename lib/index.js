...
var getEncryptCMD = function (args, map, type) {
    ...
    //cmd += ' -encode \"unicode\"';
    cmd += cloneArg(args, map, 'p.encode', 'encode');
    
    cmd += util.format(' -logto \"%s\"', path.join(process.cwd(), logger.getDefaultLogFolder(), config.moduleName + ".log"));
    return cmd;
}
...
var selectNode = '';
var startRead = false;
var filterParameter = function (line) {
    if (selectNode == '') return true;
    return startRead;
}

var isNull = function (line) {
    if (!line || line == '') {
        return true;
    }
...
var readParameterListSync = function (configFile) {
    ...
    for (var i = 0; i < lines.length; i++) {
        if (!fileHelper.isNullLineOrCommentLine(lines[i])) {
            if (filterParameter(lines[i])) {
                var out = /(.*?)=(.*)/.exec(lines[i]);
                argMap[out[1]] = out[2];

                if (selectNode == '' && out[1] == 'select') {
                    selectNode = out[2];
                }
            }
        } else if (!isNull(lines[i]) && selectNode != '' && lines[i].indexOf(selectNode) == 0) {
            startRead = true;
        } else if (!isNull(lines[i]) && startRead ===true && lines[i].indexOf('#=====') == 0) {
            startRead = false;
...
function encryptAndDecrypt(program, type, callback) {
    ...
    for (var i = 0; i < pathList.length; i++) {
        ...
        if (fs.existsSync(enPath)) {
            ...
            if (states.isFile()) {
                executeCMD(util.format('%s -file \"%s\" -out \"%s\"', cmd, enPath, enPath + config.en_suffix), callback);
            } else {
